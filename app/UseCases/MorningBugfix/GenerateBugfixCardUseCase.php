...
class GenerateBugfixCardUseCase
{
    ...
    ): array
    {
        ...
        // id検索用にキー付きコレクション化
        $problemMap = $problems->keyBy('id');
        // Gemini 用データ生成
        $items = $parsed->map(function ($tags, $id) use ($problemMap) {
            return [
                'problem_id' => $id,
                'sub_category' => $problemMap->get($id)?->subCategory?->name,
        ...
        foreach ($parsed as $problemId => $tags) {
            $subCategory = $problemMap->get($problemId)?->subCategory?->name;
